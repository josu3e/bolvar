...
class DX_Auth
{
	...
	function _init()
	{
		...
		// Pages view
		$this->template_view = $this->ci->config->item('DX_template_view');
	...
	*/
	...
	function _set_last_ip_and_last_login($user_id)
	{
		...
		{
			$data['usu_last_ip'] = $this->ci->input->ip_address();
		}

		if($this->ci->config->item('DX_login_record_time'))
		{
			$data['usu_last_login'] = date('Y-m-d H:i:s', time());
		}
			...
			if($role->rol_parent_id > 0)
			{
				...
				while ($finished == FALSE)
				{
					...
					if($i_query->num_rows() > 0)
					{
						...
						if($i_role->rol_parent_id == 0)
						{
							// Get latest parent name
							$parent_roles_name[] = $i_role->rol_name;
							// Stop looping
							$finished = TRUE;
						}
						else
						{
							...
							$parent_id = $i_role->rol_parent_id;
	...
	{
		// Get role data
		$role_data = $this->_get_role_data($data->usu_rol_id);

		// Set session data array
		$user = array(
			'DX_user_id'						=> $data->usu_id,
			'DX_username'						=> $data->usu_username,
			'DX_name'					    	=> $data->usu_nombre,
			'DX_email'					  	=> $data->usu_email,
			'DX_role_id'						=> $data->usu_rol_id,
	...
	*/
	...
	function is_admin()
	{
		return strtolower($this->ci->session->userdata('DX_role_name')) == 'administrador';
			// return strtolower($this->ci->session->userdata('DX_rol_name')) == 'administrador' || strtolower($this->ci->session->userdata('DX_rol_name')) == 'usuario';
	}
				...
				else
				{
					...
					if(crypt($password, $stored_hash) === $stored_hash)
					{
						...
						if($row->usu_newpass)
						{
							...
							$this->ci->users->clear_newpass($row->usu_id);
						}

						if($remember)
						{
							// Create auto login if user want to be remembered
							$this->_create_autologin($row->usu_id);
						}

						// Set last ip and last login
						$this->_set_last_ip_and_last_login($row->usu_id);
						// Clear login attempts
						$this->_clear_login_attempts();

						// Trigger event
						$this->ci->dx_auth_event->user_logged_in($row->usu_id);
	...
	function register_only($username, $password, $email,$fono,$nombre,$empresa,$fecnac)
	{
			// Load Models
		$this->ci->load->model('dx_auth/users', 'users');
		$this->ci->load->model('dx_auth/user_temp', 'user_temp');
		...
		$new_user = array(
			...
			'usu_password'				=> crypt($this->_encode($password)),
			'usu_email'						=> $email,
			'usu_last_ip'					=> $this->ci->input->ip_address(),
			'usu_fono'					=> $fono,
			'usu_nombre'					=> $nombre,
			'usu_empresa'    			=> $empresa,
			'usu_fec_nac'    			=> $fecnac
		...
		$new_user['usu_newpass_key'] = md5(rand().microtime());
		...
		if($login)
		{
			...
			if($query = $this->ci->users->get_login($login) AND $query->num_rows() == 1)
			{
				...
				if( ! $row->usu_newpass_key)
				{
					...
					$this->ci->users->newpass($row->usu_id, $encode, $data['key']);

					// Create reset password link to be included in email
					$data['reset_password_uri'] = site_url($this->ci->config->item('DX_reset_password_uri')."{$row->usu_username}/{$data['key']}");
					...
					$this->_email($row->usu_email, $from, $subject, $message);
	...
	function change_only_password($id,$new_pass)
	{
			// Load Models
		$this->ci->load->model('dx_auth/users', 'users');
	...
	function change_pass_admin($old_pass, $new_pass)
	{
			// Load Models
		$this->ci->load->model('dx_auth/users', 'users');
		...
		// Default return value
		$result = FAlSE;
		if($query = $this->ci->users->get_user_by_username('admin') AND $query->num_rows() > 0)
		{
			...
			if(crypt($pass, $row->usu_password) === $row->usu_password)
			{
				// Crypt and encode new password
				$new_pass = crypt($this->_encode($new_pass));

				// Replace old password with new password
				$this->ci->users->change_password($row->usu_id, $new_pass);

				// Trigger event
				$this->ci->dx_auth_event->user_changed_password($row->usu_id, $new_pass);
