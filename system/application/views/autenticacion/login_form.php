...
<?php echo form_open(substr($this->uri->uri_string(),1))?>
  <h2 class="form_header"> Porfavor inicia sesión </h2>
  ...
  <div id="operations">
		<p><?php echo form_checkbox($remember);?> <?php echo form_label('Remember me', $remember['id']);
             echo nbs(3);
             echo anchor($this->dx_auth->forgot_password_uri, 'Forgot password');?></p>
		<?php
			if ($this->dx_auth->allow_registration) {
				echo "<p>".anchor($this->dx_auth->register_uri, 'Register')."</p>";
			};
		?>
    <p><?php echo form_submit('login','Acceder');
      echo nbs(3);
      echo anchor(base_url(), 'Cancelar');?></p>
  </div>
<?php echo form_close()?>
