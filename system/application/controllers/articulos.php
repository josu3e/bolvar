...
class articulos extends Controller {
    ...
    function enviar_cotizacion() {
        ...
        $to = 'publimatrix@gmail.com';

        $subject = 'Solicitud de cotizacion';

        $cart = $this->session->userdata('cart');
        $message = 'El cliente: ' . $this->session->userdata('DX_username') . ' a solicitado la cotizaci&oacute;n siguiente: ' . "\n" . '
      ...
      <td align="center"> ' . $art[2] . "</td></tr> \n";
        ...
        if ($to != '') {
            ...
            $msj = 'su cotizaci&oacute;n fue enviada correctamente.<br/><br/>En breves momentos estar&aacute; recibiendo la cotizaci&oacute;n solicitada a su correo electr&oacute;nico.';
        } else {
    ...
    function _email($from, $to, $subject, $message) {
        ...
        //$email->from($from);
        $email->from('webmaster@gsp-peru.com', 'Webmaster');
        $email->to($to);
